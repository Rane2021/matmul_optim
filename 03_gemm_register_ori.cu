...
__global__ void Sgemm( 
    ...
    // shared memory
    __shared__ float As[2][BLOCK_SIZE_K][BLOCK_SIZE_M];  // size=2* 128*8
    __shared__ float Bs[2][BLOCK_SIZE_K][BLOCK_SIZE_N];  // size=2* 128*8
    // registers for C
    ...
    float ldg_a_reg[4*ldg_num_a];  // size= 4*1
    float ldg_b_reg[4*ldg_num_b];  // size= 4*1
    ...
    const int A_TILE_ROW_START = tid / A_TILE_THREAD_PER_ROW;  // 0-128
    const int B_TILE_ROW_START = tid / B_TILE_THREAD_PER_ROW;  // 0-8

    const int A_TILE_COL = tid % A_TILE_THREAD_PER_ROW * 4;  // 0/4
    const int B_TILE_COL = tid % B_TILE_THREAD_PER_ROW * 4;  // 0-31 * 4

    // row stride that thread uses to load multiple rows of a tile
    const int A_TILE_ROW_STRIDE = THREAD_NUM_PER_BLOCK / A_TILE_THREAD_PER_ROW;  // 128
    const int B_TILE_ROW_STRIDE = THREAD_NUM_PER_BLOCK / B_TILE_THREAD_PER_ROW;  // 8

    A = &A[(BLOCK_SIZE_M * by)* K];  // 这个操作很重要： 对 A B 分大块（如左上图）
    B = &B[BLOCK_SIZE_N * bx];

    //transfer first tile from global mem to shared mem
    // load A from global memory to shared memory
    #pragma unroll
    for ( int i = 0 ; i < BLOCK_SIZE_M ; i += A_TILE_ROW_STRIDE) {  // 只循环一次？？
        int ldg_index = i / A_TILE_ROW_STRIDE * 4;  // 0
